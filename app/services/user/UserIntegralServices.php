...
class UserIntegralServices extends BaseServices
{
    ...
    public function doClearExpireIntegral(array $uids)
    {
        ...
        foreach ($uids as $uid) {
            ...
            if ($userSumIntegral) {
                ...
                //记录清除积分
                $userBillServices->income('system_clear_integral', $uid, $number, $user_data['integral'], $uid);
	...
	* @return bool
	...
	public function newcomerGiveIntegral(int $uid)
	{
		...
		/** @var UserBillServices $userBillServices */
		$userBillServices = app()->make(UserBillServices::class);
		$userBillServices->income('newcomer_give_integral', $uid, $integral, $balance);
	...
	* @return bool
	...
	public function levelGiveIntegral(int $uid)
	{
		...
		/** @var UserBillServices $userBillServices */
		$userBillServices = app()->make(UserBillServices::class);
		$userBillServices->income('level_give_integral', $uid, $integral, $balance);
		$userServices->update($uid, ['integral' => $balance]);
		return true;
	}
}

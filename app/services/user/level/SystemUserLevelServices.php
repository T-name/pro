    ...
    public function getDiscount(int $uid, int $id)
    {
        ...
        if ($id) {
            ...
            if (!$level) {//用户存在等级ID 但是被删除或者下架重新检测升级
                /** @var UserLevelServices $userLeveleServices */
                $userLeveleServices = app()->make(UserLevelServices::class);
                $userLeveleServices->detection($uid);
                /** @var UserServicesr $userServices */
                $userServices = app()->make(UserServices::class);
                $level = $this->dao->getOne(['id' => $userServices->value(['uid' => $uid], 'level'), 'is_del' => 0, 'is_show' => 1], 'id,discount');
            }

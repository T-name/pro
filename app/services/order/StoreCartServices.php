...
class StoreCartServices extends BaseServices
{
	...
	* @throws \think\db\exception\ModelNotFoundException
    ...
    public function getUserCartCount(int $uid, string $numType = '0', int $store_id = 0)
    {
		...
		$cartNums = [];
        $sum_price = 0;
        $cartList = $this->dao->getUserCartList(['uid' => $uid, 'status' => 1], 'id,cart_num,product_id');
        if ($cartList) {
			/** @var StoreProductServices $storeProductServices */
			$storeProductServices = app()->make(StoreProductServices::class);
			$productInfos = $storeProductServices->getColumn([['id', 'in', array_column($cartList, 'product_id')]] , 'id,type,relation_id', 'id');
			foreach ($cartList as $cart) {
				$productInfo = $productInfos[$cart['product_id']] ?? [];
				if (!$productInfo) continue;
				if ($store_id) {//某门店加入购物车商品数量
					if (in_array($productInfo['type'], [0, 2]) || ($productInfo['type'] == 1 && $productInfo['relation_id'] == $store_id)) {
						$ids[] = $cart['id'];

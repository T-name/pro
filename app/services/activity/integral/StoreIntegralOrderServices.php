...
class StoreIntegralOrderServices extends BaseServices
{
     ...
     * @throws \Exception
     */
    public function createOrder($uid, $addressId, $mark, $userInfo, $num, $productInfo, $customForm)
    {
    ...
    public function deductIntegral(array $userInfo, $priceIntegral, int $uid, string $orderId)
    {
        ...
        if ($userInfo['integral'] > 0) {
            ...
            $res3 = $userBillServices->income('storeIntegral_use_integral', $uid, $priceIntegral, $integral, $orderId);
...
//                                $remark .= '，卡号：' . $virtual['card_no'] . '；密码：' . $virtual['card_pwd'] . ';';
                    ...
                    //增加发送站内信
                    $SystemMessageServices->systemSend($orderInfo['uid'], [
                        'mark' => 'virtual_info',
                        'title' => $title,
                        'content' => $content
                    ]);

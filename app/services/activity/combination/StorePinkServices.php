...
use app\jobs\pink\AuthPinkFail;
...
//            $openid && $this->pinkSuccessSend($orderInfo, $openid, $product['title'], $pink);
    ...
    public function joinPinkSuccessSend(array $order, string $openid, string $title, $pink)
    {
        if ($order['is_channel'] == 1) {
            ...
            WechatTemplateJob::dispatchDo('sendOrderPinkUseSuccess', [$openid, $order['order_id'], $title, $order['pink_id']]);
        }
        return true;
    }
    ...
    public function pinkSuccessSend(array $order, string $openid, string $title, $pink)
    {
        if ($order['is_channel'] == 1) {
            ...
            WechatTemplateJob::dispatchDo('sendOrderPinkOpenSuccess', [$openid, $pink, $title]);
        }
        return true;
    }
        ...
        try {
            ...
            if (request()->isRoutine()) {
                ...
                if (!$imageInfo) {
                    ...
                    $res = MiniProgram::appCodeUnlimit($valueData, 'pages/activity/goods_combination_status/index', 280);

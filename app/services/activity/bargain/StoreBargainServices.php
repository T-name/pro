...
//                $valueNew[$count]['quota'] = $sukValue[$suk]['quota'] ? intval($sukValue[$suk]['quota']) : 0;
	...
	*/
    public function getBargain(int $uid, int $id)
    {
        ...
        if (!$storeInfo) throw new ValidateException('砍价商品不存在');
        $this->dao->addBargain($id, 'look');
        $storeInfo['time'] = time();
        if ($storeInfo['stop_time'] < time()) throw new ValidateException('砍价已结束');
		$user = [];
		if ($uid) {
			/** @var UserServices $userServices */
			$userServices = app()->make(UserServices::class);
			$user = $userServices->getUserInfo($uid, 'uid,nickname,avatar');
			if (!$user) {
		...
		if ($uid) {
			...
			$count = $bargainUserServices->count(['uid' => $uid, 'bargain_id' => $id, 'type' => 1]);

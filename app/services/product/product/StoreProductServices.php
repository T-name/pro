...
class StoreProductServices extends BaseServices
{
    ...
    public function getStoreBranchInfo(int $id, int $store_id)
    {
        ...
        $productInfo = $this->dao->getBranchProduct(['pid' => $id, 'type' => 1, 'relation_id' => $store_id]);
...
//        $header[] = ['title' => '会员价', 'slot' => 'vip_price', 'align' => 'center', 'minWidth' => 140];
    ...
    public function save(int $id, array $data, int $type = 0, int $relation_id = 0)
    {
        ...
        [$skuList, $id, $is_new, $data] = $this->transaction(function () use ($id, $storeLabelId, $data, $description, $cate_id, $storeDescriptionServices, $storeProductAttrServices, $storeProductCouponServices, $detail, $attr, $coupon_ids, $storeDiscountProduct, $productVirtual, $slider_image) {
        ...
        event('product.create', [$id, $data, $cate_id, $storeLabelId, $skuList, $is_new, $slider_image, $description, $is_copy]);
    }
...
//            }
    ...
    public function getProductPromotions(array $list, int $promotions_type = 0)
    {
        ...
        $field = 'id,name,desc,promotions_type,title,product_id,product_partake_type,discount,discount_type,start_time,stop_time';
        [$promotionsArr, $productDetails, $promotionsDetail] = $storePromotionsServices->getProductsPromotionsDetail($productIds, $promotions_type, $field, $with);
    ...
    public function productDetail(Request $request, int $id, int $type, int $promotions_type = 0)
    {
        $uid = (int)$request->uid();
        $data['uid'] = $uid;
        $storeInfo = $this->dao->getOne(['id' => $id, 'is_show' => 1, 'is_del' => 0], '*', ['descriptions', 'brand']);
        if (!$storeInfo) {
            throw new ValidateException('商品不存在');
        } else {
        ...
        $configData = SystemConfigService::more(['site_url', 'tengxun_map_key', 'store_self_mention', 'routine_contact_type', 'site_name', 'share_qrcode', 'store_func_status']);
...
//        $storeInfo['code_base'] = $qrcodeService->getWechatQrcodePath($id . '_product_detail_wap.jpg', '/pages/goods_details/index?id=' . $id);
        ...
        $memberCardService = app()->make(MemberCardServices::class);
        $vipStatus = $memberCardService->isOpenMemberCardCache('vip_price') && sys_config('svip_price_status', 1);
        $data['storeInfo'] = array_merge($data['storeInfo'], $this->getMinPrice($uid, $data['storeInfo'], null, false));
        ...
        if (isset($infoDiy['is_reply']) && $infoDiy['is_reply']) {
            ...
            $reply = $storeProductReplyService->getRecProductReply($id, (int)($infoDiy['reply_num'] ?? 1));
    ...
    public function productIdByBrandName($productId, $productInfo = [])
    {
        if ($productInfo) {
            ...
            $brand_id = $this->dao->value(['id' => $productId], 'brand_id');
        }
        /** @var StoreBrandServices $storeBrandServices */
        $storeBrandServices = app()->make(StoreBrandServices::class);
    ...
    public function getDeliveryType(int $type, int $relation_id, array $delivery_type)
    {
        ...
        if (!sys_config('store_func_status', 1)) {
            ...
            if ($type === 1 && $relation_id) {
                /** @var SystemStoreServices $storeServices */
                $storeServices = app()->make(SystemStoreServices::class);
                $is_store = $storeServices->value(['id' => $relation_id, 'is_show' => 1, 'is_del' => 0], 'is_store');
                $store_mention = $is_store === 1;
            }
            //判断当前商品配送方式

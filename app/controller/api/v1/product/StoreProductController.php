...
//                        }
	...
	* @throws ModelNotFoundException
	...
	public function activity(Request $request, StoreCouponIssueServices $couponIssueServices, StoreDiscountsServices $storeDiscountsServices, StorePromotionsServices $storePromotionsServices, DiyServices $diyServices, $id)
	{
		...
		$uid = (int)$request->uid();
		$data = ['activity' => [], 'coupons' => [], 'discounts_products' => [], 'promotions' => []];
		if ($storeInfo) {
			$diy = $diyServices->getProductDetailDiy();
			if (isset($diy['is_activity']) && $diy['is_activity'] && !($storeInfo['type'] > 0 && !$storeInfo['pid']))
				$data['activity'] = $this->services->getActivityList($storeInfo, false);
			if (isset($diy['is_coupon']) && $diy['is_coupon'] && !($storeInfo['type'] == 1 && $storeInfo['pid'] == 0))
			...
			if (isset($diy['is_promotions']) && $diy['is_promotions'] && !($storeInfo['type'] > 0 && !$storeInfo['pid'])){
				$ids = $storeInfo['pid'] ? [$storeInfo['pid']] : [$id];
				$data['promotions'] = $storePromotionsServices->getProductsPromotions($ids, $promotions_type, '*', [], 'promotions_type');
			}
		}

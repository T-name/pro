<template>
  <div>
    ...
    <div class="main">
      ...
      <Card :bordered="false" dis-hover class="ivu-mt mr20">
        <div class="tree">
          <div class="main-btn">
            <Dropdown
              @on-click="
            ...
            >
              <a href="javascript:void(0)">
                <Button class="mb10" type="primary" @click="add" icon="md-add">新增</Button>
              </a>
              <template #list>
                <DropdownMenu>
                  <DropdownItem name="1">新增接口</DropdownItem>
                  <DropdownItem name="4">文件名</DropdownItem>
                </DropdownMenu>
              </template>
            </Dropdown>
          </div>
          ...
          >
            <template v-slot:leafNameDisplay="slotProps">
              <div></div>
              <div class="tree-node" @click.stop="onClick(slotProps.model)">
                <span class="line1" :class="{ open: formValidate.id == slotProps.model.id }">{{
                ...
                >
                  ...
                  <template #list>
                    <DropdownMenu>
                      ...
                      <DropdownItem name="2" v-if="!slotProps.model.method">编辑文件名</DropdownItem>
            ...
            <template v-slot:treeNodeIcon="slotProps">
              <span
                ...
                :style="{
                  'background-color':
                    slotProps.model.id == formValidate.id ? methodsColor(slotProps.model.method) : '#fff',
                  color: slotProps.model.id == formValidate.id ? '#fff' : methodsColor(slotProps.model.method),
      ...
      <Card :bordered="false" dis-hover class="ivu-mt right-card">
        <div class="data" v-if="formValidate.id >= 0 || formValidate.pid">
          <div class="eidt-sub">
            <div class="name">
              {{ formValidate.name }}
            </div>
    ...
    <Modal v-model="nameModal" title="分组名称" :loading="loading" @on-ok="asyncOK">
      <Input v-model="value" placeholder="请输入分组名称" style="width: 300px" />
    </Modal>
  </div>
</template>

<script>
import { interfaceList, interfaceDet, interfaceSave, interfaceEditName, interfaceDel } from '@/api/setting';

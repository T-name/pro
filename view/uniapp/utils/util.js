...
export default {
	 ...
	 * 
	 */
	PosterCanvas: function(fontColor, themeColor, siteName, arr2, store_name, price, ot_price, successFn) {
		let that = this;
		uni.showLoading({
			title: '海报生成中',
			mask: true
		...
		uni.getImageInfo({
			...
			success: function(res) {
				...
				ctx.fillText('品牌官方 · 交易保障 · 优质口碑 · 售后无忧', WIDTH/2, 65);
				ctx.save();
				...
				for (let m = 0; m < contentArray.length; m++) {
					if (m) {
						ctx.fillText(contentArray[m], 30, 955 + contentHh * m + 18);
					} else {
						ctx.fillText(contentArray[m], 30, 955 + contentHh * m);
					}
				}
				ctx.save();
				ctx.setTextAlign('left')
				...
				ctx.fillText('￥' + ot_price, 30, 1040 + contentHh);
				...
				underline(ctx, '￥' + ot_price, 30, 1032, 32, '#999', 2, 0);
				ctx.save();
				ctx.setTextAlign('left')
				ctx.setFontSize(55);
				ctx.setFillStyle(fontColor);
				ctx.fillText('￥' + price, 20, 1110 + contentHh);
				ctx.save();
				ctx.setFillStyle('#F5F5F5');
				ctx.fillRect(0, 1200, WIDTH, 113);
				ctx.setFontSize(30);
				ctx.setFillStyle('#999');
	...
	bargainPosterCanvas: function(arr2, title, label, msg, price, wd, hg, successFn) {
		...
		uni.getImageInfo({
			...
			success: function(res) {
				...
				for (let m = 0; m < contentArray.length; m++) {
					if (m) {
						ctx.fillText(contentArray[m], wd * msgx, 35 + contentHh * m + 18, 1100);
					} else {
						ctx.fillText(contentArray[m], wd * msgx, 35, 1100);
					}
				}
				// 标签内容
				ctx.setTextAlign('left')

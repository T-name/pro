	...
	<view  :style="colorStyle">
		<view class='order-submission'>
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<view class='settlement' style='z-index:100' @tap.stop="goPay" v-if="(((valid_count>0&&!discount_id) || (valid_count==cartInfo.length&&discount_id)) && shippingType) || (!shippingType && addressId) || product_type != 0">立即结算</view>
		...
		methods: {
			...
			SubOrder: function(e) {
				...
				for (var i = 0; i < that.confirm.length; i++) {
					...
					if (data.status || (data.label !== 'img' && data.value.trim())) {

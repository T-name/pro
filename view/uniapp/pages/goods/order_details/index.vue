	...
	<view :style="colorStyle">
		<view class='order-details'>
			...
			<view v-if="isGoodsReturn==false">
				...
				<view class='address' v-if="(orderInfo.shipping_type === 1 || orderInfo.shipping_type === 3) && orderInfo.product_type==0">
					<view class='name'>{{orderInfo.real_name}}<text class='phone'>{{orderInfo.user_phone}}</text></view>
					<view>{{orderInfo.user_address}}</view>
				</view>
				<view class='address' v-if="(orderInfo.shipping_type === 2 || orderInfo.shipping_type === 4) && orderInfo.product_type==0">
			...
			<view class="delivery acea-row row-between-wrapper" v-if="orderInfo.delivery_type == 'city_delivery' || orderInfo.delivery_type == 'send'">
				...
				<navigator :url="'/pages/goods/delivery_detail/index?orderId='+orderInfo.id" hover-class='none' class="details" >查看详情</navigator>
			</view>
			...
			<orderGoods :evaluate='evaluate' :giveData="giveData" :deliveryType="orderInfo.shipping_type" :statusType="status.type"  
				:sendType="orderInfo.delivery_type" :orderId="order_id" :oid="Number(orderInfo.id)" :cartInfo="cartInfo" :pid="pid" :jump="true" :refund_status="orderInfo.refund_status" :paid="Number(orderInfo.paid)" :productType="orderInfo.product_type" :giveCartInfo="giveCartInfo">
			</orderGoods>
			<!-- #ifdef H5 || APP-PLUS -->
			...
			<view class='wrapper' v-if="isGoodsReturn && orderInfo.product_type==0">
				...
				<view v-if="this.orderInfo.shipping_type !== 2 && this.orderInfo.shipping_type !== 4" class='item acea-row row-between'>
					<view>收货地址：</view>
			...
			<view class='footer acea-row row-right row-middle'
				...
				<view
					...
					class='bnt cancel' v-else-if="orderInfo.is_apply_refund && orderInfo.refund_status == 0 && cartInfo.length>1">
					批量退款</view>
				<!-- #ifdef MP -->
				<!-- <view
				...
				<view class='bnt bg-color' v-if="orderInfo.type==3 && orderInfo.paid && orderInfo.refund_status==0" @tap='goJoinPink'>查看拼团</view>

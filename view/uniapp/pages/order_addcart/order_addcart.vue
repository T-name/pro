			...
			<view class='footer acea-row row-between-wrapper' :class="isFooter?'':'on'"
				...
				<view class='money acea-row row-middle' v-if="footerswitch==true && discountInfo.deduction">
					<view class="left">
						...
						<view class="detailed" @click="discountTap" v-if="(Object.prototype.toString.call(discountInfo.coupon) === '[object Object]' || discountInfo.deduction.first_order_price || discountInfo.deduction.promotions_price) && selectValue.length">优惠明细<text class="iconfont" :class="discountInfo.discount?'icon-xiangxia':'icon-xiangshang'"></text></view>
					</view>
					<form @submit="subOrder">
						<button class='placeOrder bg-color' :class="selectValue.length?'':'on'" formType="submit">{{Object.prototype.toString.call(discountInfo.coupon) === "[object Array]" || discountInfo.coupon.used?'立即':'领券'}}下单</button>
					</form>
				</view>

		...
		<view class="group-wrapper" :class="conStyle?'borderRadius20':''" :style="'background:'+bgColor+';margin:0 '+prConfig*2+'rpx;margin-top:'+ mbConfig*2 +'rpx;'" v-if="combinationList.length>0">
			<view class="hd">
				<view class="left">
					...
					<view class="name" :style="'color:'+titleColor">拼团活动</view>
				...
				<scroll-view scroll-x="true" class="scroll" show-scrollbar="false">
					<navigator class="group-item" :style="'margin-right:'+ lrConfig*2 +'rpx;'" v-for="(item,index) in combinationList" :key="index"
					 :url="'/pages/activity/goods_combination_details/index?id='+item.id" hover-class="none">
						...
						<view class="people" v-if="joinShow">{{item.people}}人参团</view>
						<view class="info">
							...
							<view class="price-box">
								<text v-if="pinkShow" class="tips" :style="'background-color:'+txtColor+';color:'+themeColor+';'">拼团价</text>
								<text v-if="priceShow" class="price" :style="'color:'+fontColor+';'"><text>￥</text>{{item.price}}</text>
							</view>
						</view>
						<view v-if="bntShow" class="bom-btn" :style="'background-color:'+themeColor+';'">参与拼团</view>
					</navigator>
				</scroll-view>
			</view>
		</view>

			...
			<view class="navbar" :style="{ height: navH + 'rpx', opacity: opacity, paddingTop: sysHeight }">
				...
				<view id="home" class="home acea-row row-center-wrapper" :class="[opacity>0.5?'on':'']"
										...
										<view class="share acea-row row-between row-bottom" v-if="promotions_type !=1">
											<view class="money font-color skeleton-rect">
												...
												<text class="vip-money"
													v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.is_vip == 1 && diyProduct.price_type.indexOf('1') != -1">￥{{ storeInfo.vip_price }}</text>
												<image
													v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.is_vip == 1 && diyProduct.price_type.indexOf('1') != -1"
													src="../../static/images/vip.png"></image>
												<view class="level acea-row row-middle" v-if="storeInfo.level_name && diyProduct.price_type.indexOf('2') != -1">
													<view class="vip-money">￥{{storeInfo.level_price}}</view>
													<view class="icon">
					...
					<view>客服</view>
						...
						<!-- 属性组件 -->
						<productWindow :attr="attr" :isShow="1" :iSplus="1" :storeInfo="storeInfo" @myevent="onMyEvent" @ChangeAttr="ChangeAttr"

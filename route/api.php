...
Route::group('api', function () {
	...
	*/
	...
	//门店店员订单操作类
    Route::group(function () {
        Route::get('store/staff/info', 'store.StoreStaffController/info')->name('storeStaffInfo');//门店店员信息
        Route::get('store/staff/statistics', 'store.StoreStaffController/statistics')->name('storeStatistics');//门店|店员统计信息
        Route::get('store/staff/data/:type', 'store.StoreStaffController/data');//详细信息列表
        Route::get('store/order/statistics', 'store.StoreOrderController/statistics')->name('adminOrderStatistics');//订单数据统计
        Route::get('store/order/data', 'store.StoreOrderController/data')->name('adminOrderData');//订单每月统计数据
        Route::get('store/staff/card/code', 'store.StoreStaffController/code');//详细信息列表

        Route::get('store/delivery/info', 'store.StoreDeliveryController/info')->name('storeStaffInfo');//配送员信息
        Route::get('store/delivery/statistics', 'store.StoreDeliveryController/statistics')->name('deliveryStatistics');//门店配送员统计信息
        Route::get('store/delivery/data', 'store.StoreDeliveryController/data')->name('deliveryData');//每月配送统计列表数据
        Route::get('store/delivery/order', 'store.StoreDeliveryController/orderList')->name('deliveryOrder');//配送员订单列表
        Route::get('store/delivery/list', 'store.StoreDeliveryController/getDeliveryAll')->name('getDeliveryAll');//获取配送员列表
        Route::get('store/order/list', 'store.StoreOrderController/lst')->name('storeOrderList');//订单列表
        Route::get('store/refund/list', 'store.StoreOrderController/refundList')->name('refundList');//订单列表
        Route::get('store/refund/detail/:id', 'store.StoreOrderController/refundDetail')->name('refundDetail');//订单列表
        Route::get('store/order/detail/:id', 'store.StoreOrderController/detail')->name('storeOrderDetail');//订单详情
        Route::get('store/order/delivery_info/:orderId', 'store.StoreOrderController/deliveryInfo')->name('storeOrderDeliveryInfo');//订单发货获取订单信息
        Route::post('store/order/delivery/:id', 'store.StoreOrderController/delivery')->name('storeOrderDelivery');//订单发货
        Route::get('store/order/split_cart_info/:id', 'store.StoreOrderController/split_cart_info')->name('StoreOrderSplitCartInfo');//获取订单可拆分商品列表
        Route::put('store/order/split_delivery/:id', 'store.StoreOrderController/split_delivery')->name('StoreOrderSplitDelivery');//拆单发送货
        Route::post('store/order/price', 'store.StoreOrderController/price')->name('storeOrderPrice');//订单改价
        Route::post('store/order/remark', 'store.StoreOrderController/remark')->name('storeOrderRemark');//订单备注
        Route::post('store/refund/remark', 'store.StoreOrderController/refundRemark')->name('refundRemark');//订单备注
        Route::post('store/order/offline', 'store.StoreOrderController/offline')->name('storeOrderOffline');//订单支付
        Route::post('store/order/refund', 'store.StoreOrderController/refund')->name('storeOrderRefund');//订单退款
        Route::post('store/order/refund_agree/:id', 'store.StoreOrderController/agreeRefund')->name('storeOrderAgreeRefund');//商家同意退货退款
        Route::get('store/order/export_temp', 'store.StoreOrderController/getExportTemp')->name('storeOrderExportTemp');//获取电子面单模板获取
        Route::get('store/order/export_all', 'store.StoreOrderController/getExportAll')->name('storeOrderExportAll');//获取物流公司
        Route::post('store/order/cancel/:id', 'store.StoreOrderController/cancel')->name('storeOrderCancel'); //订单取消
        Route::delete('store/order/del/:id', 'store.StoreOrderController/del')->name('storeOrderDel');//订单删除
        Route::get('store/order/writeoff_info/:type', 'store.StoreOrderController/writeoffOrderinfo')->name('writeoffOrderinfo');//扫码核销获取订单信息
        Route::get('store/order/cart_info/:type', 'store.StoreOrderController/orderCartInfo')->name('writeoffOrderCartInfo');//核销获取商品信息
        Route::post('store/order/writeoff/:type', 'store.StoreOrderController/wirteoff')->name('storeOrderWriteoff');//订单核销
...
//    Route::post('like/del', 'user.UserController/like_del')->name('likeDel');//取消点赞
        ...
        Route::post('cart/add', 'v1.order.StoreCartController/add')->name('cartAdd'); //购物车添加
